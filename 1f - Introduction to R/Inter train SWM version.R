...
?mean
# F1
#  -----> over there in the help tab

# where can I see installed libraries? 
#  -----> over there in the packages tab

df <- mtcars



# quick overview of the other useful tabs and options
# environment
# history
# connections - will show later with UDAL
# tutorial 
...
# how to filter is null
beds_data_is_null <- beds_data |>
...
# what do we have decimals after means?
beds_data |>
  group_by(org_name) |>
...
# this is simply a display shortcut - when we actually save
# or use the data, the decimals are used
averages_bed_data <- beds_data |>
  group_by(org_name) |>
  summarise(mean_beds = mean(beds_av,
                             na.rm = TRUE),
            median_beds = median(beds_av,
...
data <- data |>
  rename(org_code = `organisation_code`)
...
###### HINT - check the order of your rename!
###### ensure we have 'org_code' as we use it alot!
...
# personally I find it best to use that as an error catch as this
# will count a NA as over 25,000
...
data <- data |>
  mutate (new_col = case_when(type == '1' ~ attendances * 2,
...
# we could do that in one mutate if we simply wanted the percentage and 
# not the total as well

data_tot_perc <- data |>
  group_by (org_code,
            period) |>
  mutate (perc_attend = (attendances / sum(attendances)) * 100) |>
  ungroup()
...
######### and returns the average and maximum number of attendances across 
# those sites by finacal year 

data_finance <- data |>
  filter(org_code %in%  c('RF4', 'Y02696' ,'RQ3'),
         type == '1') |>
  mutate(finance_year = ifelse(month(period) >= 4, 
                               year(period) + 1, 
                               year(period))) |>
...
#difference in days
difftime(date_end ,date_start , units = c("days"))

# difference in weeks
difftime(date_end ,date_start , units = c("weeks"))

# another zoo function can be used to calulate months
12 * (as.yearmon(date_end) - as.yearmon(date_start))
...
short_hospital_name <- str_to_title(short_hospital_name)
...
text <- paste0('The maximum number of attendances was ', short_hospital_name)
...
is_odd_or_even <- function (input, multi) {
  mult <- input * multi
  mod <- mult %% 2
  return (if_else(mod == 0, 'Even', 'Odd'))
}
